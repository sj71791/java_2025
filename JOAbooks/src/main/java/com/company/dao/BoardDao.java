    		  ...
    		  while(rset.next()) { //줄
    			  result.add( new BoardVO(
    					rset.getInt("bno"), rset.getString("btitle"), 
      			  		...
      			  		rset.getInt("bit"),rset.getString("bdate"), 
      			  		rset.getString("bip")
      			  		));
    		  }
...
//    3. 해당글읽기 /  해당글수정폼	- select * from board where bno=?
      public BoardVO select(int bno){ 
    	  ...
    	  try {
    		  ...
    		  //int bno, String btitle, String bcontent, String bname, int bit, String bdate, String bip
...
//    4. 조회수증가	- update board set bit=bit+1 where bno=?
      public int updateHit( int bno ) { 
    	  String sql="update board set bit=bit+1 where bno=?";
    	  int result=-1;
    	  ///////////////////////////
    	  DBManager db=new DBManager();
    	  Connection conn=null; PreparedStatement pstmt=null; //ResultSet rset=null;
